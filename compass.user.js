...
(function () {
  ...
  function applyAllModifications() {
    ...
    document.querySelectorAll(".field-group[data-card-id]").forEach((group) => {
      ...
      const exceedRatio = fieldConfig.includes("exceed")
        ? group.querySelector(".exceed-ratio")?.value
        : null;
      const excellentValue = fieldConfig.includes("excellent")
        ? group.querySelector(".excellent-value")?.value
        : null;
      ...
      if (!isRefund) {
        ...
        if (bottomItems[1] && excellentValue) {
          const formattedValue = isMoneyField(title)
            ...
            : excellentValue.toString().endsWith("%")
            ? excellentValue
            : excellentValue;
